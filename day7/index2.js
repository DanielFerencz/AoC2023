...
const whatKind = (a) => {
    ...
    let jokers = cardss.J | 0;

    if (jokers !== 0) {
        if (jokers === 5) return 10;
        if (jokers === 4) return 10;

        let maxi = 0;
        let maxiV;

        for (let key of Object.keys(cardss)) {
            if (key !== "J") {
                if (maxi < cardss[key]) {
                    maxi = cardss[key];
                    maxiV = key;
        ...
        for (let val of Object.keys(cardss)) {
            if (val !== "J") {
                if (cardss[val] === 5) {
                    return 10;
                }
    ...
    // console.log(jokers, cardss, strength, Object.keys(cardss));
